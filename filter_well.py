...
def open_filter_well_reg():
    ...
    def push_filter():
        ...
        old_reg = session.query(AnalysisReg).filter_by(id=get_regmod_id()).first()
        new_reg = AnalysisReg(title=ui.lineEdit_string.text())
        session.add(new_reg)
        session.commit()

        for old_markup in old_reg.markups:
            well = session.query(Well).filter(Well.id == old_markup.well_id).first()
            if ui_fw.checkBox_distance.isChecked():
                try:
                    distance = calc_distance(well.x_coord, well.y_coord, center_object[0], center_object[1])
        ...
        session.commit()
        update_list_reg()
        set_info(f'Скопирован регрессионный анализ - "{old_reg.title}"', 'green')
