...
def apply_cluster_tooltips(ui) -> None:
    ...
    tips = {
        ...
        "listWidget_cluster_list_log": "Каротажные параметры текущего набора. В скобках показано покрытие по скважинам: найдено/всего; [calc] означает calculator feature / расчётный признак из глобальной библиотеки.",
        "pushButton_cluster_add_well_log": "Добавить один canonical curve / каноническую кривую: через открытую форму каротажа LOG TO CLUST или через открытие form_well_log для выбранной скважины.",
        "pushButton_cluster_well_log_constructor": "CONSTR: открыть конструктор расчётных признаков Well Log для текущего набора. Глобальный признак можно добавить как [calc]; interpolation не используется, поэтому несовместимая глубинная сетка блокирует добавление и COLLECT.",
        ...
        "pushButton_clust_collect_well_log": "COLLECT: собрать итоговую таблицу data по выбранным скважинам, интервалам, canonical curves и calculator features. Зелёный цвет означает актуальные data; красный — набор не собран/изменён или расчётный признак может заблокировать сборку.",
