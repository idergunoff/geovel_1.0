# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'C:\Users\SayfutdinovaAMa\PycharmProjects\geovel_1.0\qt\torch_regressor.ui'
#
# Created by: PyQt5 UI code generator 5.15.10
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_TorchNNRegressor(object):
    def setupUi(self, TorchNNRegressor):
        TorchNNRegressor.setObjectName("TorchNNRegressor")
        TorchNNRegressor.resize(495, 659)
        self.gridLayout_11 = QtWidgets.QGridLayout(TorchNNRegressor)
        self.gridLayout_11.setObjectName("gridLayout_11")
        self.gridLayout_8 = QtWidgets.QGridLayout()
        self.gridLayout_8.setObjectName("gridLayout_8")
        self.groupBox = QtWidgets.QGroupBox(TorchNNRegressor)
        self.groupBox.setTitle("")
        self.groupBox.setObjectName("groupBox")
        self.gridLayout = QtWidgets.QGridLayout(self.groupBox)
        self.gridLayout.setObjectName("gridLayout")
        self.checkBox_choose_param = QtWidgets.QCheckBox(self.groupBox)
        self.checkBox_choose_param.setStyleSheet("background-color: rgb(252, 204, 165);")
        self.checkBox_choose_param.setObjectName("checkBox_choose_param")
        self.gridLayout.addWidget(self.checkBox_choose_param, 0, 0, 1, 2)
        self.label = QtWidgets.QLabel(self.groupBox)
        self.label.setObjectName("label")
        self.gridLayout.addWidget(self.label, 1, 0, 1, 1)
        self.doubleSpinBox_choose_lr = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.doubleSpinBox_choose_lr.setDecimals(6)
        self.doubleSpinBox_choose_lr.setSingleStep(0.01)
        self.doubleSpinBox_choose_lr.setProperty("value", 0.0001)
        self.doubleSpinBox_choose_lr.setObjectName("doubleSpinBox_choose_lr")
        self.gridLayout.addWidget(self.doubleSpinBox_choose_lr, 1, 1, 1, 1)
        self.label_2 = QtWidgets.QLabel(self.groupBox)
        self.label_2.setObjectName("label_2")
        self.gridLayout.addWidget(self.label_2, 2, 0, 1, 1)
        self.doubleSpinBox_choose_dropout = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.doubleSpinBox_choose_dropout.setDecimals(6)
        self.doubleSpinBox_choose_dropout.setSingleStep(0.01)
        self.doubleSpinBox_choose_dropout.setProperty("value", 0.01)
        self.doubleSpinBox_choose_dropout.setObjectName("doubleSpinBox_choose_dropout")
        self.gridLayout.addWidget(self.doubleSpinBox_choose_dropout, 2, 1, 1, 1)
        self.label_3 = QtWidgets.QLabel(self.groupBox)
        self.label_3.setObjectName("label_3")
        self.gridLayout.addWidget(self.label_3, 3, 0, 1, 1)
        self.doubleSpinBox_choose_decay = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.doubleSpinBox_choose_decay.setDecimals(6)
        self.doubleSpinBox_choose_decay.setSingleStep(0.01)
        self.doubleSpinBox_choose_decay.setProperty("value", 0.001)
        self.doubleSpinBox_choose_decay.setObjectName("doubleSpinBox_choose_decay")
        self.gridLayout.addWidget(self.doubleSpinBox_choose_decay, 3, 1, 1, 1)
        self.label_4 = QtWidgets.QLabel(self.groupBox)
        self.label_4.setObjectName("label_4")
        self.gridLayout.addWidget(self.label_4, 4, 0, 1, 1)
        self.doubleSpinBox_choose_reagular = QtWidgets.QDoubleSpinBox(self.groupBox)
        self.doubleSpinBox_choose_reagular.setDecimals(6)
        self.doubleSpinBox_choose_reagular.setSingleStep(0.01)
        self.doubleSpinBox_choose_reagular.setProperty("value", 0.0001)
        self.doubleSpinBox_choose_reagular.setObjectName("doubleSpinBox_choose_reagular")
        self.gridLayout.addWidget(self.doubleSpinBox_choose_reagular, 4, 1, 1, 1)
        self.label_5 = QtWidgets.QLabel(self.groupBox)
        self.label_5.setObjectName("label_5")
        self.gridLayout.addWidget(self.label_5, 5, 0, 1, 1)
        self.lineEdit_choose_layers = QtWidgets.QLineEdit(self.groupBox)
        self.lineEdit_choose_layers.setObjectName("lineEdit_choose_layers")
        self.gridLayout.addWidget(self.lineEdit_choose_layers, 5, 1, 1, 1)
        self.gridLayout_8.addWidget(self.groupBox, 0, 0, 1, 1)
        self.gridLayout_5 = QtWidgets.QGridLayout()
        self.gridLayout_5.setObjectName("gridLayout_5")
        self.label_11 = QtWidgets.QLabel(TorchNNRegressor)
        self.label_11.setObjectName("label_11")
        self.gridLayout_5.addWidget(self.label_11, 0, 0, 1, 1)
        self.spinBox_epochs = QtWidgets.QSpinBox(TorchNNRegressor)
        self.spinBox_epochs.setMaximum(10000)
        self.spinBox_epochs.setProperty("value", 100)
        self.spinBox_epochs.setObjectName("spinBox_epochs")
        self.gridLayout_5.addWidget(self.spinBox_epochs, 0, 1, 1, 1)
        self.label_12 = QtWidgets.QLabel(TorchNNRegressor)
        self.label_12.setObjectName("label_12")
        self.gridLayout_5.addWidget(self.label_12, 1, 0, 1, 1)
        self.comboBox_optimizer = QtWidgets.QComboBox(TorchNNRegressor)
        self.comboBox_optimizer.setObjectName("comboBox_optimizer")
        self.comboBox_optimizer.addItem("")
        self.comboBox_optimizer.addItem("")
        self.gridLayout_5.addWidget(self.comboBox_optimizer, 1, 1, 1, 1)
        self.label_13 = QtWidgets.QLabel(TorchNNRegressor)
        self.label_13.setObjectName("label_13")
        self.gridLayout_5.addWidget(self.label_13, 2, 0, 1, 1)
        self.comboBox_loss = QtWidgets.QComboBox(TorchNNRegressor)
        self.comboBox_loss.setObjectName("comboBox_loss")
        self.comboBox_loss.addItem("")
        self.comboBox_loss.addItem("")
        self.comboBox_loss.addItem("")
        self.comboBox_loss.addItem("")
        self.gridLayout_5.addWidget(self.comboBox_loss, 2, 1, 1, 1)
        self.label_17 = QtWidgets.QLabel(TorchNNRegressor)
        self.label_17.setObjectName("label_17")
        self.gridLayout_5.addWidget(self.label_17, 3, 0, 1, 1)
        self.comboBox_activation_func = QtWidgets.QComboBox(TorchNNRegressor)
        self.comboBox_activation_func.setObjectName("comboBox_activation_func")
        self.comboBox_activation_func.addItem("")
        self.gridLayout_5.addWidget(self.comboBox_activation_func, 3, 1, 1, 1)
        self.gridLayout_8.addLayout(self.gridLayout_5, 1, 0, 1, 1)
        self.groupBox_2 = QtWidgets.QGroupBox(TorchNNRegressor)
        self.groupBox_2.setTitle("")
        self.groupBox_2.setObjectName("groupBox_2")
        self.gridLayout_6 = QtWidgets.QGridLayout(self.groupBox_2)
        self.gridLayout_6.setObjectName("gridLayout_6")
        self.checkBox_tune_param = QtWidgets.QCheckBox(self.groupBox_2)
        self.checkBox_tune_param.setStyleSheet("background-color: rgb(251, 228, 170);")
        self.checkBox_tune_param.setObjectName("checkBox_tune_param")
        self.gridLayout_6.addWidget(self.checkBox_tune_param, 0, 0, 1, 2)
        self.label_7 = QtWidgets.QLabel(self.groupBox_2)
        self.label_7.setObjectName("label_7")
        self.gridLayout_6.addWidget(self.label_7, 1, 0, 1, 1)
        self.lineEdit_tune_lr = QtWidgets.QLineEdit(self.groupBox_2)
        self.lineEdit_tune_lr.setObjectName("lineEdit_tune_lr")
        self.gridLayout_6.addWidget(self.lineEdit_tune_lr, 1, 1, 1, 1)
        self.label_8 = QtWidgets.QLabel(self.groupBox_2)
        self.label_8.setObjectName("label_8")
        self.gridLayout_6.addWidget(self.label_8, 2, 0, 1, 1)
        self.lineEdit_tune_dropout = QtWidgets.QLineEdit(self.groupBox_2)
        self.lineEdit_tune_dropout.setObjectName("lineEdit_tune_dropout")
        self.gridLayout_6.addWidget(self.lineEdit_tune_dropout, 2, 1, 1, 1)
        self.label_6 = QtWidgets.QLabel(self.groupBox_2)
        self.label_6.setObjectName("label_6")
        self.gridLayout_6.addWidget(self.label_6, 3, 0, 1, 1)
        self.lineEdit_tune_decay = QtWidgets.QLineEdit(self.groupBox_2)
        self.lineEdit_tune_decay.setObjectName("lineEdit_tune_decay")
        self.gridLayout_6.addWidget(self.lineEdit_tune_decay, 3, 1, 1, 1)
        self.label_9 = QtWidgets.QLabel(self.groupBox_2)
        self.label_9.setObjectName("label_9")
        self.gridLayout_6.addWidget(self.label_9, 4, 0, 1, 1)
        self.doubleSpinBox_tune_regular = QtWidgets.QDoubleSpinBox(self.groupBox_2)
        self.doubleSpinBox_tune_regular.setDecimals(6)
        self.doubleSpinBox_tune_regular.setSingleStep(0.01)
        self.doubleSpinBox_tune_regular.setProperty("value", 0.01)
        self.doubleSpinBox_tune_regular.setObjectName("doubleSpinBox_tune_regular")
        self.gridLayout_6.addWidget(self.doubleSpinBox_tune_regular, 4, 1, 1, 1)
        self.label_10 = QtWidgets.QLabel(self.groupBox_2)
        self.label_10.setObjectName("label_10")
        self.gridLayout_6.addWidget(self.label_10, 5, 0, 1, 1)
        self.lineEdit_tune_layers = QtWidgets.QLineEdit(self.groupBox_2)
        self.lineEdit_tune_layers.setObjectName("lineEdit_tune_layers")
        self.gridLayout_6.addWidget(self.lineEdit_tune_layers, 5, 1, 1, 1)
        self.label_15 = QtWidgets.QLabel(self.groupBox_2)
        self.label_15.setObjectName("label_15")
        self.gridLayout_6.addWidget(self.label_15, 6, 0, 1, 1)
        self.spinBox_layers_num = QtWidgets.QSpinBox(self.groupBox_2)
        self.spinBox_layers_num.setProperty("value", 10)
        self.spinBox_layers_num.setObjectName("spinBox_layers_num")
        self.gridLayout_6.addWidget(self.spinBox_layers_num, 6, 1, 1, 1)
        self.label_16 = QtWidgets.QLabel(self.groupBox_2)
        self.label_16.setObjectName("label_16")
        self.gridLayout_6.addWidget(self.label_16, 7, 0, 1, 1)
        self.spinBox_trials = QtWidgets.QSpinBox(self.groupBox_2)
        self.spinBox_trials.setProperty("value", 10)
        self.spinBox_trials.setObjectName("spinBox_trials")
        self.gridLayout_6.addWidget(self.spinBox_trials, 7, 1, 1, 1)
        self.gridLayout_8.addWidget(self.groupBox_2, 2, 0, 1, 1)
        self.gridLayout_11.addLayout(self.gridLayout_8, 0, 0, 1, 1)
        self.gridLayout_9 = QtWidgets.QGridLayout()
        self.gridLayout_9.setObjectName("gridLayout_9")
        self.gridLayout_4 = QtWidgets.QGridLayout()
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.groupBox_17 = QtWidgets.QGroupBox(TorchNNRegressor)
        self.groupBox_17.setObjectName("groupBox_17")
        self.layoutWidget = QtWidgets.QWidget(self.groupBox_17)
        self.layoutWidget.setGeometry(QtCore.QRect(10, 20, 131, 101))
        self.layoutWidget.setObjectName("layoutWidget")
        self.gridLayout_30 = QtWidgets.QGridLayout(self.layoutWidget)
        self.gridLayout_30.setContentsMargins(0, 0, 0, 0)
        self.gridLayout_30.setObjectName("gridLayout_30")
        self.checkBox_mxabsscaler = QtWidgets.QCheckBox(self.layoutWidget)
        self.checkBox_mxabsscaler.setObjectName("checkBox_mxabsscaler")
        self.gridLayout_30.addWidget(self.checkBox_mxabsscaler, 3, 0, 1, 1)
        self.checkBox_mnmxscaler = QtWidgets.QCheckBox(self.layoutWidget)
        self.checkBox_mnmxscaler.setObjectName("checkBox_mnmxscaler")
        self.gridLayout_30.addWidget(self.checkBox_mnmxscaler, 2, 0, 1, 1)
        self.checkBox_robscaler = QtWidgets.QCheckBox(self.layoutWidget)
        self.checkBox_robscaler.setObjectName("checkBox_robscaler")
        self.gridLayout_30.addWidget(self.checkBox_robscaler, 1, 0, 1, 1)
        self.checkBox_stdscaler = QtWidgets.QCheckBox(self.layoutWidget)
        self.checkBox_stdscaler.setChecked(True)
        self.checkBox_stdscaler.setObjectName("checkBox_stdscaler")
        self.gridLayout_30.addWidget(self.checkBox_stdscaler, 0, 0, 1, 1)
        self.groupBox_12 = QtWidgets.QGroupBox(self.groupBox_17)
        self.groupBox_12.setGeometry(QtCore.QRect(10, 190, 141, 53))
        self.groupBox_12.setObjectName("groupBox_12")
        self.spinBox_reg_pca = QtWidgets.QSpinBox(self.groupBox_12)
        self.spinBox_reg_pca.setGeometry(QtCore.QRect(29, 23, 49, 20))
        self.spinBox_reg_pca.setMaximum(1000)
        self.spinBox_reg_pca.setObjectName("spinBox_reg_pca")
        self.checkBox_reg_pca = QtWidgets.QCheckBox(self.groupBox_12)
        self.checkBox_reg_pca.setGeometry(QtCore.QRect(10, 26, 16, 16))
        self.checkBox_reg_pca.setText("")
        self.checkBox_reg_pca.setObjectName("checkBox_reg_pca")
        self.checkBox_reg_pca_mle = QtWidgets.QCheckBox(self.groupBox_12)
        self.checkBox_reg_pca_mle.setGeometry(QtCore.QRect(84, 24, 39, 17))
        self.checkBox_reg_pca_mle.setObjectName("checkBox_reg_pca_mle")
        self.groupBox_10 = QtWidgets.QGroupBox(self.groupBox_17)
        self.groupBox_10.setGeometry(QtCore.QRect(10, 130, 141, 53))
        self.groupBox_10.setObjectName("groupBox_10")
        self.checkBox_reg_cross_val = QtWidgets.QCheckBox(self.groupBox_10)
        self.checkBox_reg_cross_val.setGeometry(QtCore.QRect(10, 20, 16, 16))
        self.checkBox_reg_cross_val.setText("")
        self.checkBox_reg_cross_val.setObjectName("checkBox_reg_cross_val")
        self.spinBox_reg_cross_val = QtWidgets.QSpinBox(self.groupBox_10)
        self.spinBox_reg_cross_val.setGeometry(QtCore.QRect(70, 20, 61, 20))
        self.spinBox_reg_cross_val.setMinimum(2)
        self.spinBox_reg_cross_val.setProperty("value", 5)
        self.spinBox_reg_cross_val.setObjectName("spinBox_reg_cross_val")
        self.gridLayout_4.addWidget(self.groupBox_17, 1, 0, 1, 3)
        self.checkBox_early_stop = QtWidgets.QCheckBox(TorchNNRegressor)
        self.checkBox_early_stop.setObjectName("checkBox_early_stop")
        self.gridLayout_4.addWidget(self.checkBox_early_stop, 3, 0, 1, 2)
        self.comboBox_dataset = QtWidgets.QComboBox(TorchNNRegressor)
        self.comboBox_dataset.setObjectName("comboBox_dataset")
        self.comboBox_dataset.addItem("")
        self.comboBox_dataset.addItem("")
        self.gridLayout_4.addWidget(self.comboBox_dataset, 0, 1, 1, 2)
        self.label_14 = QtWidgets.QLabel(TorchNNRegressor)
        self.label_14.setObjectName("label_14")
        self.gridLayout_4.addWidget(self.label_14, 0, 0, 1, 1)
        self.spinBox_stop_patience = QtWidgets.QSpinBox(TorchNNRegressor)
        self.spinBox_stop_patience.setProperty("value", 20)
        self.spinBox_stop_patience.setObjectName("spinBox_stop_patience")
        self.gridLayout_4.addWidget(self.spinBox_stop_patience, 3, 2, 1, 1)
        self.gridLayout_9.addLayout(self.gridLayout_4, 0, 0, 1, 1)
        self.gridLayout_7 = QtWidgets.QGridLayout()
        self.gridLayout_7.setObjectName("gridLayout_7")
        self.checkBox_save_model = QtWidgets.QCheckBox(TorchNNRegressor)
        self.checkBox_save_model.setEnabled(True)
        self.checkBox_save_model.setObjectName("checkBox_save_model")
        self.gridLayout_7.addWidget(self.checkBox_save_model, 1, 0, 1, 1)
        self.gridLayout_2 = QtWidgets.QGridLayout()
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.pushButton_train = QtWidgets.QPushButton(TorchNNRegressor)
        self.pushButton_train.setStyleSheet("background-color: rgb(252, 204, 165);")
        self.pushButton_train.setObjectName("pushButton_train")
        self.gridLayout_2.addWidget(self.pushButton_train, 0, 0, 1, 1)
        self.pushButton_lineup = QtWidgets.QPushButton(TorchNNRegressor)
        self.pushButton_lineup.setStyleSheet("background-color: rgb(252, 204, 165);")
        self.pushButton_lineup.setObjectName("pushButton_lineup")
        self.gridLayout_2.addWidget(self.pushButton_lineup, 0, 1, 1, 1)
        self.gridLayout_7.addLayout(self.gridLayout_2, 2, 0, 1, 1)
        self.groupBox_3 = QtWidgets.QGroupBox(TorchNNRegressor)
        self.groupBox_3.setObjectName("groupBox_3")
        self.layoutWidget_2 = QtWidgets.QWidget(self.groupBox_3)
        self.layoutWidget_2.setGeometry(QtCore.QRect(4, 27, 161, 48))
        self.layoutWidget_2.setObjectName("layoutWidget_2")
        self.gridLayout_12 = QtWidgets.QGridLayout(self.layoutWidget_2)
        self.gridLayout_12.setContentsMargins(0, 0, 0, 0)
        self.gridLayout_12.setObjectName("gridLayout_12")
        self.radioButton_stacking = QtWidgets.QRadioButton(self.layoutWidget_2)
        self.radioButton_stacking.setChecked(True)
        self.radioButton_stacking.setObjectName("radioButton_stacking")
        self.gridLayout_12.addWidget(self.radioButton_stacking, 1, 1, 1, 1)
        self.radioButton_voting = QtWidgets.QRadioButton(self.layoutWidget_2)
        self.radioButton_voting.setObjectName("radioButton_voting")
        self.gridLayout_12.addWidget(self.radioButton_voting, 2, 1, 1, 1)
        self.checkBox_stack_vote = QtWidgets.QCheckBox(self.layoutWidget_2)
        self.checkBox_stack_vote.setObjectName("checkBox_stack_vote")
        self.gridLayout_12.addWidget(self.checkBox_stack_vote, 0, 0, 2, 2)
        self.groupBox_16 = QtWidgets.QGroupBox(self.groupBox_3)
        self.groupBox_16.setGeometry(QtCore.QRect(0, 80, 167, 61))
        self.groupBox_16.setObjectName("groupBox_16")
        self.gridLayout_37 = QtWidgets.QGridLayout(self.groupBox_16)
        self.gridLayout_37.setObjectName("gridLayout_37")
        self.checkBox_bagging = QtWidgets.QCheckBox(self.groupBox_16)
        self.checkBox_bagging.setText("")
        self.checkBox_bagging.setObjectName("checkBox_bagging")
        self.gridLayout_37.addWidget(self.checkBox_bagging, 0, 0, 1, 1)
        self.spinBox_bagging = QtWidgets.QSpinBox(self.groupBox_16)
        self.spinBox_bagging.setMinimum(5)
        self.spinBox_bagging.setMaximum(1000)
        self.spinBox_bagging.setProperty("value", 10)
        self.spinBox_bagging.setObjectName("spinBox_bagging")
        self.gridLayout_37.addWidget(self.spinBox_bagging, 0, 1, 1, 1)
        self.gridLayout_7.addWidget(self.groupBox_3, 0, 0, 1, 1)
        self.gridLayout_9.addLayout(self.gridLayout_7, 1, 0, 1, 1)
        self.gridLayout_11.addLayout(self.gridLayout_9, 0, 1, 1, 1)

        self.retranslateUi(TorchNNRegressor)
        QtCore.QMetaObject.connectSlotsByName(TorchNNRegressor)

    def retranslateUi(self, TorchNNRegressor):
        _translate = QtCore.QCoreApplication.translate
        TorchNNRegressor.setWindowTitle(_translate("TorchNNRegressor", "TorchNNRegressor"))
        self.checkBox_choose_param.setText(_translate("TorchNNRegressor", "Choose parameters"))
        self.label.setToolTip(_translate("TorchNNRegressor", "Этот параметр контролирует размер шага, с которым обновляются веса модели во время обучения. Более высокая скорость обучения может привести к более быстрой сходимости, но также может вызвать перепрыгивание через оптимальное решение."))
        self.label.setText(_translate("TorchNNRegressor", "learning rate"))
        self.doubleSpinBox_choose_lr.setToolTip(_translate("TorchNNRegressor", "Этот параметр контролирует размер шага, с которым обновляются веса модели во время обучения. Более высокая скорость обучения может привести к более быстрой сходимости, но также может вызвать перепрыгивание через оптимальное решение."))
        self.label_2.setToolTip(_translate("TorchNNRegressor", "Это техника регуляризации, используемая для предотвращения переобучения. Она случайным образом отключает (обнуляет) определенный процент входных данных слоя во время обучения, делая модель более устойчивой."))
        self.label_2.setText(_translate("TorchNNRegressor", "dropout"))
        self.doubleSpinBox_choose_dropout.setToolTip(_translate("TorchNNRegressor", "Это техника регуляризации, используемая для предотвращения переобучения. Она случайным образом отключает (обнуляет) определенный процент входных данных слоя во время обучения, делая модель более устойчивой."))
        self.label_3.setToolTip(_translate("TorchNNRegressor", "<html><head/><body><p>L2-регуляризация - добавляет штраф к функции потерь, побуждая модель изучать более мелкие веса и снижая риск переобучения.</p></body></html>"))
        self.label_3.setText(_translate("TorchNNRegressor", "weight decay"))
        self.doubleSpinBox_choose_decay.setToolTip(_translate("TorchNNRegressor", "<html><head/><body><p>L2-регуляризация, это добавляет штраф к функции потерь, побуждая модель изучать более мелкие веса и снижая риск </p></body></html>"))
        self.label_4.setToolTip(_translate("TorchNNRegressor", "Этот гиперпараметр контролирует силу регуляризации, применяемой к весам модели, помогая предотвратить переобучение."))
        self.label_4.setText(_translate("TorchNNRegressor", "regularization"))
        self.doubleSpinBox_choose_reagular.setToolTip(_translate("TorchNNRegressor", "Этот гиперпараметр контролирует силу регуляризации, применяемой к весам модели, помогая предотвратить переобучение."))
        self.label_5.setToolTip(_translate("TorchNNRegressor", "количество и размер скрытых слоев в нейронной сети."))
        self.label_5.setText(_translate("TorchNNRegressor", "hidden layers"))
        self.lineEdit_choose_layers.setToolTip(_translate("TorchNNRegressor", "количество и размер скрытых слоев в нейронной сети."))
        self.lineEdit_choose_layers.setText(_translate("TorchNNRegressor", "50 100 150 50"))
        self.label_11.setToolTip(_translate("TorchNNRegressor", "Количество раз, когда вся обучающая выборка проходит через модель во время обучения."))
        self.label_11.setText(_translate("TorchNNRegressor", "epochs"))
        self.spinBox_epochs.setToolTip(_translate("TorchNNRegressor", "Количество раз, когда вся обучающая выборка проходит через модель во время обучения."))
        self.label_12.setToolTip(_translate("TorchNNRegressor", "<p>Алгоритм оптимизации:</p>\n"
"<p>Adam показывает хорошие результаты во многих случаях и является удачным выбором по умолчанию.</p>\n"
"<p>SGD может быть полезен, если вы работаете с очень большими наборами данных.</p>\n"
"<p>L-BFGS может быть целесообразен, если у вас небольшое количество параметров и вам необходима высокая точность.</p>"))
        self.label_12.setText(_translate("TorchNNRegressor", "optimizer"))
        self.comboBox_optimizer.setToolTip(_translate("TorchNNRegressor", "<p>Алгоритм оптимизации:</p>\n"
"<p>Adam показывает хорошие результаты во многих случаях и является удачным выбором по умолчанию.</p>\n"
"<p>SGD может быть полезен, если вы работаете с очень большими наборами данных.</p>\n"
"<p>L-BFGS может быть целесообразен, если у вас небольшое количество параметров и вам необходима высокая точность.</p>"))
        self.comboBox_optimizer.setItemText(0, _translate("TorchNNRegressor", "Adam"))
        self.comboBox_optimizer.setItemText(1, _translate("TorchNNRegressor", "SGD"))
        self.label_13.setToolTip(_translate("TorchNNRegressor", "<html><head/><body><p>Функция, используемая для вычисления ошибки между предсказаниями и истинными метками:</p><p><span style=\" font-weight:600;\">MSE (средняя квадратическая ошибка)</span> — это оценка среднего значения квадрата ошибок, различие между предсказанием и фактическим значением. Эту метрику удобно использовать для выявления аномалий.</p><p><span style=\" font-weight:600;\">MAE (средняя абсолютная ошибка) </span>— это оценка того, насколько близки предсказания к фактическим значениями. Эта метрика менее чувствительна к выбросам и может дать общее представление о качестве модели.</p><p><span style=\" font-weight:600;\">Функция потерь Хьюбера</span> — функция потерь, используемая в устойчивой регрессии, которая менее чувствительна к выбросам, чем квадратичная ошибка. Использует L2 для малых ошибок и L1 — для больших.</p><p><span style=\" font-weight:600;\">Сглаженная L1-потеря </span>практически эквивалентна потере Хьюбера с дополнительным бета-параметром. </p></body></html>"))
        self.label_13.setText(_translate("TorchNNRegressor", "loss function"))
        self.comboBox_loss.setToolTip(_translate("TorchNNRegressor", "<html><head/><body><p>Функция, используемая для вычисления ошибки между предсказаниями и истинными метками:</p><p><span style=\" font-weight:600;\">BCELoss (Binary Cross-Entropy Loss)</span>: Это функция потерь, используемая для задач двоичной классификации</p><p><span style=\" font-weight:600;\">BCEWithLogitsLoss</span>: Этот вариант функции потерь объединяет вычисление двоичной перекрестной энтропии с применением сигмоидной активации на выходном слое. </p><p><span style=\" font-weight:600;\">CrossEntropyLoss</span>: Эта функция потерь используется для задач многоклассовой классификации, где выходы модели должны быть вероятностями принадлежности к каждому классу.</p>"))
        self.comboBox_loss.setItemText(0, _translate("TorchNNRegressor", "MSE"))
        self.comboBox_loss.setItemText(1, _translate("TorchNNRegressor", "MAE"))
        self.comboBox_loss.setItemText(2, _translate("TorchNNRegressor", "HuberLoss"))
        self.comboBox_loss.setItemText(3, _translate("TorchNNRegressor", "SmoothL1Loss"))
        self.label_17.setToolTip(_translate("TorchNNRegressor", "<p>Функция активации, используемая в скрытых слоях нейронной сети</p>\n"
"<ul><li><b>ReLU</b> является отличным выбором по умолчанию и хорошо работает во многих случаях <i>(возвращает значение x, если x больше 0, и 0 в противном случае)<i/>.</li>\n"
"<li><b>Sigmoid</b> лучше всего подходит для двоичной классификации, где необходимо получить вероятностные прогнозы <i>(S-образная функция, которая отображает входные значения в диапазон от 0 до 1)</i>.</li>\n"
"<li><b>Tanh</b> может быть предпочтительнее, чем sigmoid, когда требуется, чтобы выходы были центрированы вокруг нуля <i>(похожая на сигмоид функция, но она отображает входные значения в диапазон от -1 до 1)</i>. </li></ul>"))
        self.label_17.setText(_translate("TorchNNRegressor", "activation func"))
        self.comboBox_activation_func.setToolTip(_translate("TorchNNRegressor", "<p>Функция активации, используемая в скрытых слоях нейронной сети</p>\n"
"<ul><li><b>ReLU</b> является отличным выбором по умолчанию и хорошо работает во многих случаях <i>(возвращает значение x, если x больше 0, и 0 в противном случае)<i/>.</li>\n"
"<li><b>Sigmoid</b> лучше всего подходит для двоичной классификации, где необходимо получить вероятностные прогнозы <i>(S-образная функция, которая отображает входные значения в диапазон от 0 до 1)</i>.</li>\n"
"<li><b>Tanh</b> может быть предпочтительнее, чем sigmoid, когда требуется, чтобы выходы были центрированы вокруг нуля <i>(похожая на сигмоид функция, но она отображает входные значения в диапазон от -1 до 1)</i>. </li></ul>"))
        self.comboBox_activation_func.setItemText(0, _translate("TorchNNRegressor", "ReLU"))
        self.checkBox_tune_param.setText(_translate("TorchNNRegressor", "Tune parameters"))
        self.label_7.setToolTip(_translate("TorchNNRegressor", "Этот параметр контролирует размер шага, с которым обновляются веса модели во время обучения. Более высокая скорость обучения может привести к более быстрой сходимости, но также может вызвать перепрыгивание через оптимальное решение."))
        self.label_7.setText(_translate("TorchNNRegressor", "learning rate"))
        self.lineEdit_tune_lr.setToolTip(_translate("TorchNNRegressor", "Этот параметр контролирует размер шага, с которым обновляются веса модели во время обучения. Более высокая скорость обучения может привести к более быстрой сходимости, но также может вызвать перепрыгивание через оптимальное решение."))
        self.lineEdit_tune_lr.setText(_translate("TorchNNRegressor", "0,0001-0,01"))
        self.label_8.setToolTip(_translate("TorchNNRegressor", "Это техника регуляризации, используемая для предотвращения переобучения. Она случайным образом отключает (обнуляет) определенный процент входных данных слоя во время обучения, делая модель более устойчивой."))
        self.label_8.setText(_translate("TorchNNRegressor", "dropout"))
        self.lineEdit_tune_dropout.setToolTip(_translate("TorchNNRegressor", "Это техника регуляризации, используемая для предотвращения переобучения. Она случайным образом отключает (обнуляет) определенный процент входных данных слоя во время обучения, делая модель более устойчивой."))
        self.lineEdit_tune_dropout.setText(_translate("TorchNNRegressor", "0,01-0,9"))
        self.label_6.setToolTip(_translate("TorchNNRegressor", "<html><head/><body><p>L2-регуляризация - добавляет штраф к функции потерь, побуждая модель изучать более мелкие веса и снижая риск переобучения.</p></body></html>"))
        self.label_6.setText(_translate("TorchNNRegressor", "weight decay"))
        self.lineEdit_tune_decay.setToolTip(_translate("TorchNNRegressor", "<html><head/><body><p>L2-регуляризация - добавляет штраф к функции потерь, побуждая модель изучать более мелкие веса и снижая риск переобучения.</p></body></html>"))
        self.lineEdit_tune_decay.setText(_translate("TorchNNRegressor", "0,001-0,2"))
        self.label_9.setToolTip(_translate("TorchNNRegressor", "Этот гиперпараметр контролирует силу регуляризации, применяемой к весам модели, помогая предотвратить переобучение."))
        self.label_9.setText(_translate("TorchNNRegressor", "regularization"))
        self.doubleSpinBox_tune_regular.setToolTip(_translate("TorchNNRegressor", "Этот гиперпараметр контролирует силу регуляризации, применяемой к весам модели, помогая предотвратить переобучение."))
        self.label_10.setText(_translate("TorchNNRegressor", "hidden layers"))
        self.lineEdit_tune_layers.setText(_translate("TorchNNRegressor", "150-550"))
        self.label_15.setText(_translate("TorchNNRegressor", "hidden layers num"))
        self.label_16.setToolTip(_translate("TorchNNRegressor", "Количество циклов обучения или итераций, которые необходимо выполнить."))
        self.label_16.setText(_translate("TorchNNRegressor", "num trials"))
        self.groupBox_17.setTitle(_translate("TorchNNRegressor", "PreProcess"))
        self.checkBox_mxabsscaler.setText(_translate("TorchNNRegressor", "MaxAbsScaler"))
        self.checkBox_mnmxscaler.setText(_translate("TorchNNRegressor", "MinMaxScaler"))
        self.checkBox_robscaler.setText(_translate("TorchNNRegressor", "RobustScaler"))
        self.checkBox_stdscaler.setText(_translate("TorchNNRegressor", "StandardScaler"))
        self.groupBox_12.setTitle(_translate("TorchNNRegressor", "PCA"))
        self.checkBox_reg_pca_mle.setText(_translate("TorchNNRegressor", "mle"))
        self.groupBox_10.setTitle(_translate("TorchNNRegressor", "Cross Validate"))
        self.checkBox_early_stop.setToolTip(_translate("TorchNNRegressor", "<p>Эта опция позволяет автоматически останавливать обучение, когда производительность модели на проверочном наборе данных перестает улучшаться. Это помогает предотвратить переобучение.</p>"))
        self.checkBox_early_stop.setText(_translate("TorchNNRegressor", "Early stopping"))
        self.comboBox_dataset.setItemText(0, _translate("TorchNNRegressor", "shuffle"))
        self.comboBox_dataset.setItemText(1, _translate("TorchNNRegressor", "well split"))
        self.label_14.setText(_translate("TorchNNRegressor", "dataset:"))
        self.checkBox_save_model.setText(_translate("TorchNNRegressor", "save"))
        self.pushButton_train.setText(_translate("TorchNNRegressor", "TRAIN"))
        self.pushButton_lineup.setText(_translate("TorchNNRegressor", "ADD TO LINEUP"))
        self.groupBox_3.setTitle(_translate("TorchNNRegressor", "Stacking/Voting"))
        self.radioButton_stacking.setText(_translate("TorchNNRegressor", "Stacking"))
        self.radioButton_voting.setText(_translate("TorchNNRegressor", "Voting"))
        self.checkBox_stack_vote.setText(_translate("TorchNNRegressor", "ON"))
        self.groupBox_16.setTitle(_translate("TorchNNRegressor", "Bagging"))


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    TorchNNRegressor = QtWidgets.QWidget()
    ui = Ui_TorchNNRegressor()
    ui.setupUi(TorchNNRegressor)
    TorchNNRegressor.show()
    sys.exit(app.exec_())
